...
type IProvider = ethers.providers.ExternalProvider

const instance = getCurrentInstance()
const auth = instance?.appContext.config.globalProperties.$auth as Web3AuthModalPack
...
async function onSignIn() {
  try {
    await auth.signIn() // const { eoa, safes } = await auth.signIn() // when using safe-auth-kit
    provider = new ethers.providers.Web3Provider(auth.getProvider() as IProvider)
  }
  catch (e) {
